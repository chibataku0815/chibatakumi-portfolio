...
// SeededSettleJumpFinishDemo — the seeded-settle-jump schedule rendered live with
...
// the article's own (see ./finish/seeded-settle-jump-source.ts) — no deliverable-parity
// claim is made.
//
...
import { SeededSettleJumpDemo } from "./SeededSettleJumpDemo";
import {
  drawSeededSettleSourceFrame,
  FINISH_RENDER_SIZE,
  FINISH_STREAM_NAMESPACE,
} from "./finish/seeded-settle-jump-source";
import {
  SEEDED_SETTLE_FPS,
...
export function SeededSettleJumpFinishDemo() {
    ...
    (async () => {
      try {
        ...
        const renderFrame = (frame: number) => {
          drawSeededSettleSourceFrame(c2d, size, frame);
        ...
        const tick = (now: number) => {
          ...
          const frame = Math.floor(((now - start) / 1000) * SEEDED_SETTLE_FPS) %
            SEEDED_SETTLE_PERIOD_FRAMES;
        ...
        const startOrStop = () => {
          ...
          if (reduceQuery.matches) {
            renderFrame(SEEDED_SETTLE_POSTER_FRAME);
          } else {
            raf = requestAnimationFrame(tick);
          }
        };
  ...
  if (fallback) return <SeededSettleJumpDemo />;
