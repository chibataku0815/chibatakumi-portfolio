...
// MasterRotationEchoFinishDemo — the master-rotation-echo schedule rendered
// live with the API-finish standard (grain + chromatic aberration, light
// palette) on WebGPU.
//
...
// ./finish/master-rotation-echo-source.ts) — no deliverable-parity claim is made.
//
// Degradation chain: WebGPU unavailable / init failure → the plain SVG demo (no finish);
// prefers-reduced-motion → a single finished still inside the fast window.
...
import { MasterRotationEchoDemo } from "./MasterRotationEchoDemo";
import {
  drawMasterRotationEchoSourceFrame,
  FINISH_RENDER_SIZE,
  FINISH_STREAM_NAMESPACE,
} from "./finish/master-rotation-echo-source";
import {
...
export function MasterRotationEchoFinishDemo() {
    ...
    (async () => {
      try {
        ...
        const renderFrame = (frame: number) => {
          drawMasterRotationEchoSourceFrame(c2d, size, frame);
        ...
        const tick = (now: number) => {
          ...
          const frame = Math.floor(((now - start) / 1000) * MASTER_ROTATION_ECHO_FPS) %
            MASTER_ROTATION_ECHO_PERIOD_FRAMES;
        ...
        const startOrStop = () => {
          ...
          if (reduceQuery.matches) {
            renderFrame(MASTER_ROTATION_ECHO_FAST_FRAME);
          } else {
            raf = requestAnimationFrame(tick);
          }
        };
  ...
  if (fallback) return <MasterRotationEchoDemo />;
