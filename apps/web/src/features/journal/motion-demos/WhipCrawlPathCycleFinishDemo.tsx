...
// WhipCrawlPathCycleFinishDemo — the whip-crawl-path-cycle schedule rendered
...
// the article's own (see ./finish/whip-crawl-path-cycle-source.ts) — no
// deliverable-parity claim is made.
//
// Degradation chain: WebGPU unavailable / init failure → the plain SVG demo (no finish);
// prefers-reduced-motion → a single finished still at the mid-crawl poster frame.
...
import { WhipCrawlPathCycleDemo } from "./WhipCrawlPathCycleDemo";
import {
  drawWhipCrawlSourceFrame,
  FINISH_RENDER_SIZE,
  FINISH_STREAM_NAMESPACE,
} from "./finish/whip-crawl-path-cycle-source";
import {
  WHIP_CRAWL_FPS,
...
export function WhipCrawlPathCycleFinishDemo() {
    ...
    (async () => {
      try {
        ...
        const renderFrame = (frame: number) => {
          drawWhipCrawlSourceFrame(c2d, size, frame);
        ...
        const tick = (now: number) => {
          ...
          const frame = Math.floor(((now - start) / 1000) * WHIP_CRAWL_FPS) %
            WHIP_CRAWL_PERIOD_FRAMES;
        ...
        const startOrStop = () => {
          ...
          if (reduceQuery.matches) {
            renderFrame(WHIP_CRAWL_POSTER_FRAME);
          } else {
            raf = requestAnimationFrame(tick);
          }
        };
  ...
  if (fallback) return <WhipCrawlPathCycleDemo />;
