...
// AutoOrientFinishDemo — the ring-orbit-3d schedule rendered live with the
...
// the article's own (see ./finish/ring-orbit-3d-source.ts) — no deliverable-parity claim.
//
...
import { AutoOrientDemo } from "./AutoOrientDemo";
import {
  drawRingOrbitSourceFrame,
  FINISH_RENDER_SIZE,
  FINISH_STREAM_NAMESPACE,
} from "./finish/ring-orbit-3d-source";
import {
  RING_ORBIT_FPS,
...
export function AutoOrientFinishDemo() {
    ...
    (async () => {
      try {
        ...
        const renderFrame = (frame: number) => {
          drawRingOrbitSourceFrame(c2d, size, frame);
        ...
        const tick = (now: number) => {
          ...
          const frame = Math.floor(((now - start) / 1000) * RING_ORBIT_FPS) %
            RING_ORBIT_PERIOD_FRAMES;
        ...
        const startOrStop = () => {
          ...
          if (reduceQuery.matches) {
            renderFrame(RING_ORBIT_POSTER_FRAME);
          } else {
            raf = requestAnimationFrame(tick);
          }
        };
  ...
  if (fallback) return <AutoOrientDemo />;
