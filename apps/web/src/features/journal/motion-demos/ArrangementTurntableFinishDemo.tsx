...
// ArrangementTurntableFinishDemo — the arrangement-turntable schedule rendered
...
// the article's own (see ./finish/arrangement-turntable-source.ts) — no deliverable-parity
// claim is made.
//
...
import { ArrangementTurntableDemo } from "./ArrangementTurntableDemo";
import {
  drawArrangementSourceFrame,
  FINISH_RENDER_SIZE,
  FINISH_STREAM_NAMESPACE,
} from "./finish/arrangement-turntable-source";
import {
  ARRANGEMENT_FPS,
...
export function ArrangementTurntableFinishDemo() {
    ...
    (async () => {
      try {
        ...
        const renderFrame = (frame: number) => {
          drawArrangementSourceFrame(c2d, size, frame);
        ...
        const tick = (now: number) => {
          ...
          const frame = Math.floor(((now - start) / 1000) * ARRANGEMENT_FPS) %
            ARRANGEMENT_PERIOD_FRAMES;
        ...
        const startOrStop = () => {
          ...
          if (reduceQuery.matches) {
            renderFrame(ARRANGEMENT_POSTER_FRAME);
          } else {
            raf = requestAnimationFrame(tick);
          }
        };
  ...
  if (fallback) return <ArrangementTurntableDemo />;
