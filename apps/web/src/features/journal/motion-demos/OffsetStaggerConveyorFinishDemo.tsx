...
// OffsetStaggerConveyorFinishDemo — the offset schedule rendered live with the
...
// the article's own (see ./finish/offset-stagger-conveyor-source.ts) — no deliverable-parity
// claim is made.
//
...
import { OffsetStaggerConveyorDemo } from "./OffsetStaggerConveyorDemo";
import {
  drawOffsetStaggerConveyorSourceFrame,
  FINISH_RENDER_SIZE,
  FINISH_STREAM_NAMESPACE,
} from "./finish/offset-stagger-conveyor-source";
import {
  OFFSET_FPS,
  OFFSET_POSTER_FRAME,
...
export function OffsetStaggerConveyorFinishDemo() {
    ...
    (async () => {
      try {
        ...
        const renderFrame = (frame: number) => {
          drawOffsetStaggerConveyorSourceFrame(c2d, size, frame);
        ...
        const tick = (now: number) => {
          ...
          const frame = Math.floor(((now - start) / 1000) * OFFSET_FPS) %
            OFFSET_PERIOD_FRAMES;
        ...
        const startOrStop = () => {
          ...
          if (reduceQuery.matches) {
            renderFrame(OFFSET_POSTER_FRAME);
          } else {
            raf = requestAnimationFrame(tick);
          }
        };
  ...
  if (fallback) return <OffsetStaggerConveyorDemo />;
