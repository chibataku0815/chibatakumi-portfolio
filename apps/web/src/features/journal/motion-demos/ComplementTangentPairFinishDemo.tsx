...
// ComplementTangentPairFinishDemo — the complement-tangent-pair schedule
// rendered live with the API-finish standard (grain + chromatic aberration,
// light palette) on WebGPU.
//
...
// own (see ./finish/complement-tangent-pair-source.ts) — no deliverable-parity claim
// is made.
//
// Degradation chain: WebGPU unavailable / init failure → the plain SVG demo (no finish);
// prefers-reduced-motion → a single finished still at the rest frame.
...
import { ComplementTangentPairDemo } from "./ComplementTangentPairDemo";
import {
  drawComplementTangentPairSourceFrame,
  FINISH_RENDER_SIZE,
  FINISH_STREAM_NAMESPACE,
} from "./finish/complement-tangent-pair-source";
import {
...
export function ComplementTangentPairFinishDemo() {
    ...
    (async () => {
      try {
        ...
        const renderFrame = (frame: number) => {
          drawComplementTangentPairSourceFrame(c2d, size, frame);
        ...
        const tick = (now: number) => {
          ...
          const frame =
            Math.floor(((now - start) / 1000) * COMPLEMENT_TANGENT_FPS) %
            COMPLEMENT_TANGENT_PERIOD_FRAMES;
        ...
        const startOrStop = () => {
          ...
          if (reduceQuery.matches) {
            renderFrame(COMPLEMENT_TANGENT_REST_FRAME);
          } else {
            raf = requestAnimationFrame(tick);
          }
        };
  ...
  if (fallback) return <ComplementTangentPairDemo />;
