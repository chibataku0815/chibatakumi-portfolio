...
// CoupledShearRotationFinishDemo — the coupled-shear-rotation schedule
...
// the article's own (see ./finish/coupled-shear-rotation-source.ts) — no
// deliverable-parity claim is made.
//
// Degradation chain: WebGPU unavailable / init failure → the plain SVG demo (no finish);
// prefers-reduced-motion → a single finished still at the peak-separation poster frame.
...
import { CoupledShearRotationDemo } from "./CoupledShearRotationDemo";
import {
  drawCoupledShearSourceFrame,
  FINISH_RENDER_SIZE,
  FINISH_STREAM_NAMESPACE,
} from "./finish/coupled-shear-rotation-source";
import {
  COUPLED_SHEAR_FPS,
...
export function CoupledShearRotationFinishDemo() {
    ...
    (async () => {
      try {
        ...
        const renderFrame = (frame: number) => {
          drawCoupledShearSourceFrame(c2d, size, frame);
        ...
        const tick = (now: number) => {
          ...
          const frame = Math.floor(((now - start) / 1000) * COUPLED_SHEAR_FPS) %
            COUPLED_SHEAR_PERIOD_FRAMES;
        ...
        const startOrStop = () => {
          ...
          if (reduceQuery.matches) {
            renderFrame(COUPLED_SHEAR_POSTER_FRAME);
          } else {
            raf = requestAnimationFrame(tick);
          }
        };
  ...
  if (fallback) return <CoupledShearRotationDemo />;
