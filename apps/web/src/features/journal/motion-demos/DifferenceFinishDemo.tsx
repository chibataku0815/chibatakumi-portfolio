...
// DifferenceFinishDemo — the difference schedule rendered live with the API-finish
...
// the article's own (see ./finish/difference-source.ts) — no deliverable-parity claim.
//
...
import {
  drawDifferenceSourceFrame,
  FINISH_RENDER_SIZE,
  FINISH_STREAM_NAMESPACE,
} from "./finish/difference-source";
import {
  DIFFERENCE_FPS,
...
export function DifferenceFinishDemo() {
    ...
    (async () => {
      try {
        ...
        const renderFrame = (frame: number) => {
          drawDifferenceSourceFrame(c2d, size, frame);
        ...
        const tick = (now: number) => {
          ...
          const frame = Math.floor(((now - start) / 1000) * DIFFERENCE_FPS) %
            DIFFERENCE_PERIOD_FRAMES;
        ...
        const startOrStop = () => {
          ...
          if (reduceQuery.matches) {
            renderFrame(DIFFERENCE_POSTER_FRAME);
          } else {
            raf = requestAnimationFrame(tick);
          }
        };
  ...
  if (fallback) return <DifferenceDemo />;
