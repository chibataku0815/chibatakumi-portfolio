...
// PlanarToSolidFinishDemo — the disc-tumble-projection schedule rendered live with
// the API-finish standard (grain + chromatic aberration, light palette) on WebGPU.
//
// Architecture mirrors the lab deliverable pipeline: the verb draws a source frame
// (canvas2d = the Remotion base render's analogue, three-tone body/face/wall), then
// @bridges/webgpu-finish applies the finish as two WGSL passes whose math is
// parity-proven against the vec-core CPU oracle. The grain stream id is the article's
// own (see ./finish/disc-tumble-projection-source.ts) — no deliverable-parity claim.
//
...
import { PlanarToSolidDemo } from "./PlanarToSolidDemo";
import {
  drawDiscTumbleSourceFrame,
  FINISH_RENDER_SIZE,
  FINISH_STREAM_NAMESPACE,
} from "./finish/disc-tumble-projection-source";
import {
  DISC_TUMBLE_FPS,
...
export function PlanarToSolidFinishDemo() {
    ...
    (async () => {
      try {
        ...
        const renderFrame = (frame: number) => {
          drawDiscTumbleSourceFrame(c2d, size, frame);
        ...
        const tick = (now: number) => {
          ...
          const frame = Math.floor(((now - start) / 1000) * DISC_TUMBLE_FPS) %
            DISC_TUMBLE_PERIOD_FRAMES;
        ...
        const startOrStop = () => {
          ...
          if (reduceQuery.matches) {
            renderFrame(DISC_TUMBLE_POSTER_FRAME);
          } else {
            raf = requestAnimationFrame(tick);
          }
        };
  ...
  if (fallback) return <PlanarToSolidDemo />;
