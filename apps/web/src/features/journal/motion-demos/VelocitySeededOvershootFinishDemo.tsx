...
// VelocitySeededOvershootFinishDemo — the velocity-seeded-overshoot schedule
...
// the article's own (see ./finish/velocity-seeded-overshoot-source.ts) — no
// deliverable-parity claim is made.
//
// Degradation chain: WebGPU unavailable / init failure → the plain SVG demo (no finish);
// prefers-reduced-motion → a single finished still at the arrival poster frame.
...
import { VelocitySeededOvershootDemo } from "./VelocitySeededOvershootDemo";
import {
  drawVsoSourceFrame,
  FINISH_RENDER_SIZE,
  FINISH_STREAM_NAMESPACE,
} from "./finish/velocity-seeded-overshoot-source";
import {
  VSO_FPS,
  VSO_POSTER_FRAME,
  VSO_PERIOD_FRAMES,
...
export function VelocitySeededOvershootFinishDemo() {
    ...
    (async () => {
      try {
        ...
        const renderFrame = (frame: number) => {
          drawVsoSourceFrame(c2d, size, frame);
        ...
        const tick = (now: number) => {
          ...
          const frame = Math.floor(((now - start) / 1000) * VSO_FPS) %
            VSO_PERIOD_FRAMES;
  ...
  if (fallback) return <VelocitySeededOvershootDemo />;
