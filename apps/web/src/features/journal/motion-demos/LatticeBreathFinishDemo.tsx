...
// LatticeBreathFinishDemo — the lattice-breath schedule rendered live with the API-finish
...
// oracle (see docs/journal/motion-demo-webgpu-finish-plan.md — hop-1: oracle ≡ lab bytes,
// hop-2: WGSL ≡ oracle). Grain reseeds per frame with the SAME seed strings as the posted
// SNS clip, so the field sequence is bit-identical to the deliverable's.
//
// Degradation chain: WebGPU unavailable / init failure → the plain SVG demo (no finish);
// prefers-reduced-motion → a single finished still at the peak frame.
...
} from "./finish/lattice-breath-source";
import {
  LATTICE_BREATH_FPS,
...
export function LatticeBreathFinishDemo() {
    ...
    (async () => {
      try {
        ...
        const tick = (now: number) => {
          ...
          const frame = Math.floor(((now - start) / 1000) * LATTICE_BREATH_FPS) %
            LATTICE_BREATH_PERIOD_FRAMES;
        ...
        const startOrStop = () => {
          ...
          if (reduceQuery.matches) {
            renderFrame(LATTICE_BREATH_PEAK_FRAME);
          } else {
            raf = requestAnimationFrame(tick);
          }
        };
  ...
  if (fallback) return <LatticeBreathDemo />;
