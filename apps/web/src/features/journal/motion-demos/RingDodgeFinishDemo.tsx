...
// RingDodgeFinishDemo — the ring-dodge schedule rendered live with the API-finish
...
// the article's own (see ./finish/ring-dodge-source.ts) — no deliverable-parity claim.
//
...
import { RingDodgeDemo } from "./RingDodgeDemo";
import {
  drawRingDodgeSourceFrame,
  FINISH_RENDER_SIZE,
  FINISH_STREAM_NAMESPACE,
} from "./finish/ring-dodge-source";
import {
  RING_DODGE_FPS,
...
export function RingDodgeFinishDemo() {
    ...
    (async () => {
      try {
        ...
        const renderFrame = (frame: number) => {
          drawRingDodgeSourceFrame(c2d, size, frame);
        ...
        const tick = (now: number) => {
          ...
          const frame = Math.floor(((now - start) / 1000) * RING_DODGE_FPS) %
            RING_DODGE_PERIOD_FRAMES;
        ...
        const startOrStop = () => {
          ...
          if (reduceQuery.matches) {
            renderFrame(RING_DODGE_POSTER_FRAME);
          } else {
            raf = requestAnimationFrame(tick);
          }
        };
  ...
  if (fallback) return <RingDodgeDemo />;
