...
// TangencyCoupledDriveFinishDemo — the tangency-coupled-drive schedule rendered
// live with the API-finish standard (grain + chromatic aberration, light palette)
// on WebGPU.
//
...
// ./finish/tangency-coupled-drive-source.ts) — no deliverable-parity claim is made.
//
// Degradation chain: WebGPU unavailable / init failure → the plain SVG demo (no finish);
// prefers-reduced-motion → a single finished still at the diamond frame.
...
import { TangencyCoupledDriveDemo } from "./TangencyCoupledDriveDemo";
import {
  drawTangencyCoupledDriveSourceFrame,
  FINISH_RENDER_SIZE,
  FINISH_STREAM_NAMESPACE,
} from "./finish/tangency-coupled-drive-source";
import {
  TANGENCY_DRIVE_FPS,
...
export function TangencyCoupledDriveFinishDemo() {
    ...
    (async () => {
      try {
        ...
        const renderFrame = (frame: number) => {
          drawTangencyCoupledDriveSourceFrame(c2d, size, frame);
        ...
        const tick = (now: number) => {
          ...
          const frame = Math.floor(((now - start) / 1000) * TANGENCY_DRIVE_FPS) %
            TANGENCY_DRIVE_PERIOD_FRAMES;
        ...
        const startOrStop = () => {
          ...
          if (reduceQuery.matches) {
            renderFrame(TANGENCY_DRIVE_DIAMOND_FRAME);
          } else {
            raf = requestAnimationFrame(tick);
          }
        };
  ...
  if (fallback) return <TangencyCoupledDriveDemo />;
