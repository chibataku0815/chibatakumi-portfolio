// Source-frame renderer for the ring-dodge finish demo — the canvas2d analogue
// of the lab's Remotion base render, in the API-finish standard light palette.
// 9 circles (one linear orbiter + 8 ring dots dodging on a blind pulse clock),
// all in the single elem ink.
//
// Shared by the live demo (RingDodgeFinishDemo) so the GPU pipeline consumes
...
export const FINISH_STREAM_NAMESPACE = "journal-ring-dodge";

export const drawRingDodgeSourceFrame = (
  ctx: CanvasRenderingContext2D,
  size: number,
  frame: number,
): void => {
  const scale = size / RING_DODGE_VIEWBOX;
  ...
  ringDodgeDotsAt(frame).forEach((d) => {
    ctx.beginPath();
