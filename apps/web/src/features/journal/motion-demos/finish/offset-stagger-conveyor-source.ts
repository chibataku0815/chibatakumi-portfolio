// Source-frame renderer for the offset finish demo — the canvas2d analogue of
// the lab's Remotion base render, in the API-finish standard light palette.
// Circles on one horizontal baseline, sizes walking the slot-key ladder, the
// same clip replayed at staggered time offsets, all in the single elem ink.
//
// Shared by the live demo (OffsetStaggerConveyorFinishDemo) so the GPU pipeline
...
export const FINISH_STREAM_NAMESPACE = "journal-offset-stagger-conveyor";

export const drawOffsetStaggerConveyorSourceFrame = (
  ctx: CanvasRenderingContext2D,
  size: number,
  frame: number,
): void => {
  const scale = size / OFFSET_VIEWBOX;
