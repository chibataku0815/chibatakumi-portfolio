// Source-frame renderer for the parallax-bob finish demo — the canvas2d analogue
// of the lab's Remotion base render, in the API-finish standard light palette.
// Seven circles sharing one vertical bob, all in the single elem ink.
//
// Shared by the live demo (ParallaxBobFinishDemo) so the GPU pipeline consumes
...
export const FINISH_STREAM_NAMESPACE = "journal-parallax-bob";

export const drawParallaxSourceFrame = (
  ctx: CanvasRenderingContext2D,
  size: number,
  frame: number,
): void => {
  const scale = size / PARALLAX_VIEWBOX;
  ...
  parallaxDotsAt(frame).forEach((d) => {
    ctx.beginPath();
