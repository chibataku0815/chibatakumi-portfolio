...
import { TemporalEchoResidueSurface } from "./TemporalEchoResidueSurface";
import { temporalEchoResidueFixtures } from "./fixtures";

type TemporalEchoResidueReferenceWorkProps = {
  autoPlay?: boolean;
  captureMode?: boolean;
  frameOverride?: number | null;
};

export function TemporalEchoResidueReferenceWork({
  autoPlay = true,
  captureMode = false,
  frameOverride = null,
}: TemporalEchoResidueReferenceWorkProps) {
  if (captureMode) {
    return (
      <main className="min-h-screen bg-[var(--bg-dark)] px-2 py-2 text-[var(--text-base)] sm:px-3 sm:py-3">
        <div className="mx-auto max-w-7xl">
          <TemporalEchoResidueSurface
            autoPlay={autoPlay}
            captureMode
            ...
            <div className="space-y-6">
              <section>
                ...
                <p className="mt-3 text-sm leading-relaxed text-[var(--text-muted)]">
                  {temporalEchoResidueFixtures.runtimeLabel}
                </p>
              </section>
              ...
              <section>
                ...
                <ul className="mt-3 space-y-2 text-sm text-[var(--text-base)]">
                  {temporalEchoResidueFixtures.techniqueFamily.map((item) => (
                    <li
                      key={item}
              ...
              <section>
                ...
                <ul className="mt-3 space-y-2 text-sm text-[var(--text-base)]">
                  {temporalEchoResidueFixtures.extractionTargets.map((target) => (
                    <li
                      key={target}
                      className="border-l border-white/15 pl-3 font-mono text-[12px] tracking-[0.12em]"
                    >
              ...
              <section>
                ...
                <p className="mt-3 text-sm leading-relaxed text-[var(--text-muted)]">
                  {temporalEchoResidueFixtures.subjectLabel}
                </p>
              </section>
              ...
              <section>
                ...
                <ul className="mt-3 space-y-2 text-sm leading-relaxed text-[var(--text-muted)]">
                  {temporalEchoResidueFixtures.nonGoals.map((item) => (
                    <li key={item}>{item}</li>
                  ))}
                </ul>
              </section>
