 ...
 *
 * 子ども向けの説明:
 * - 画面上はつまみが 3 本だけでも、裏では色・グレイン・ハレーション・ブルームなど **複数の数値**がまとめて動きます。
 * - Pro モードではそれぞれの数値を **1 本ずつ**いじれます。
 *
 * 制限:
 * - いまの数値から「つまみがどの位置か」を逆計算するときは **だいたいの位置**になります（Pro で細かく触ったあと薄くズレうる）。
 */
 ...
 * 他の軸が触るキーはそのままにし、`current` をベースにマージして使う。
 *
 * @param axis - どのメタか
 * @param value01 - つまみ位置（0〜1）
 * @returns `MERGE_PARAMS` に渡す部分更新
 */
export function quickMetaPatchForValue(axis: QuickMetaAxis, value01: number): Partial<Params> {
  const t = clamp01(value01);
  if (axis === "filmLook") {
    return {
      /** 銘柄感: 1 を中心に上下に振る（プリセットの後からでも破綻しにくい） */
      saturation: clamp(1 + (t - 0.5) * 2.2, 0, 3),
      /** 色かぶりは ControlPanel 実スライダーと同じ -1〜1 に収める */
      temperature: clamp((t - 0.5) * 1.4, -1, 1),
      /** ControlPanel の filmGrain 上限 0.5 に合わせる */
  ...
  if (axis === "era") {
    return {
      ...
      /** ハレ色は Quick では動かさない（Pro の色相スライダーを尊重） */
    };
  }
  return {
