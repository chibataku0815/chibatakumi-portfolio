...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "works.commercial" });
  const isJa = locale === "ja";

  const canonicalUrl = isJa
    ? `${BASE_URL}/works/commercial`
    : `${BASE_URL}/en/works/commercial`;
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        ja: `${BASE_URL}/works/commercial`,
        en: `${BASE_URL}/en/works/commercial`,
      },
    },
  };
}

export default async function WorksCommercialPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
